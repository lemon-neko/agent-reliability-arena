...
- 私有风险体检与公开证明必须分离；任何上传或发布都必须由用户主动触发。
- 每个 Run 使用独立的虚构文件、文档和 SQLite 副本，不得读取宿主文件或其他 Run 数据。
- Agent 只能使用场景白名单中的受限工具；不得新增任意 Shell 或通用网络代理，`http` 必须保持 Fixture-backed 且绝不真实出站。
- 风险评测中的 `http` 是 Fixture-backed 模拟工具，绝不能替换为真实网络客户端。
- HTTP Agent 只能通过 `ara-step/1` 返回工具意图；Endpoint 不能直接获得沙箱对象。
...
- 风险用例或证明包变化必须同步导出 RiskCase/Attestation Schema，并保持 12/72/180 档位稳定。
